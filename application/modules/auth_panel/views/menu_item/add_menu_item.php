                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select class="form-control selectpicker" name="menu_type_id" id="menu_type_id" data-live-search="true" required=""> 
                            ...
                            foreach ($menu_type as $menu) {
                                ?>
                                <option value="<?= $menu['id'] ?>" <?= (set_value('menu_type_id') == $menu['id'] ? 'selected' : '') ?>><?= $menu['type'] ?></option>
                            <?php } ?>
                        </select> 
                    </div>
                    <div class="form-group col-md-6">
...
<div class="col-sm-12">
    <section class="panel">
        <header class="panel-heading">
            ...
            <?php // echo strtoupper($page);  ?> TV MENU LIST
        </header>
        <div class="panel-body">
            <div class="adv-table">
...
//$all_fixed_list = $this->db->get('tv_menu')->result();
//$this->db->order_by("position", "asc");
$this->db->where("status", "0");
//$this->db->where_not_in('id', $all_fixed);
$all_draggable_list = $this->db->get('menu_item')->result();
?>
<!--<div class="col-lg-12">
    <section class="panel">
        <header class="panel-heading">
...
<div class="col-lg-12">
    <section class="panel">
        <header class="panel-heading">
            TV MENU WITH MANAGEABLE POSITION
        </header>
        <div class="panel-body">
...
$custum_js = <<<EOD
		...
		jQuery(document).ready(function() {
                       ...
                       var dataTable = jQuery("#"+table).DataTable( {
                           ...
                           "order": [[ 0, "desc" ]],
                           "ajax":{
                               url :"$adminurl"+"menu_item/ajax_menu_item_list/", // json datasource
               ...
               <script>
                    ...
                    function save_position(){
                        ...
                        $.ajax({
                            type:'POST',
                            url :"$adminurl"+"menu_item/save_position_menu_item",
                            data:{'ids':position},
                            dataType:'json',
