...
class country_price extends MX_Controller {
...
public function amazon_s3_upload($name, $aws_path) {
    ...
    public function country() {
        if ($this->input->post()) {
            $this->form_validation->set_rules('country', 'Validity', 'trim|required');
            $this->form_validation->set_rules('currency', 'Amount', 'trim|required|numeric');
            $this->form_validation->set_rules('price', 'Plan Name', 'trim|required');
            if ($this->form_validation->run() == FALSE) {
            } else {
                $insert_data = array(
                    'c_name' => $this->input->post('country'), //in days...
    ...
    public function delete_premium_plan($id) {
        $delete_user = $this->Premium_video_model->delete_premium_plan($id);
    ...
    public function ajax_premium_plan_list() {
        ...
        $query = "SELECT count(id) as total
				  FROM premium_plan where status=0";
        ...
        $sql = "SELECT id,validity,amount,plan_name,total_devices
				FROM premium_plan where status=0";
